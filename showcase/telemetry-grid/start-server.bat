@echo off
REM TelemetryGrid Showcase HTTP Server
...
REM Navigate to project root (two levels up from showcase\telemetry-grid)
cd /d "%~dp0..\.."
...
echo Open showcase at: http://localhost:%PORT%/showcase/telemetry-grid/
echo.
start "" "http://localhost:%PORT%/showcase/telemetry-grid/"
