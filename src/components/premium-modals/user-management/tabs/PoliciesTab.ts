...
export class PoliciesTab {
  ...
  private buildPolicyItem(p: GCDRPolicy): HTMLElement {
    ...
    const riskBadge = p.riskLevel ? `<span class="gm-badge" style="${riskColor}">${p.riskLevel}</span>` : '';
          ...
          ${
            p.allow?.length
              ? p.allow
                  .map(
                    (a) =>
                      `<code style="display:block;font-size:11px;color:var(--um-badge-user-text);padding:2px 0;">${this.esc(a)}</code>`
                  )
                  .join('')
  ...
  private showPolicyForm(existing: GCDRPolicy | null): void {
    ...
    modal.innerHTML = `
      <h4 style="margin:0 0 16px;font-size:15px;font-weight:600;color:var(--um-text-primary); font-family: inherit;">${isEdit ? 'Editar' : 'Nova'} Política</h4>
    ...
    modal.querySelector('.pol-save')!.addEventListener('click', async () => {
      ...
      const allow = modal
        .querySelector<HTMLTextAreaElement>('[name=allow]')!
        .value.split('\n')
        .map((s) => s.trim())
        .filter(Boolean);
      const deny = modal
        .querySelector<HTMLTextAreaElement>('[name=deny]')!
        .value.split('\n')
        .map((s) => s.trim())
        .filter(Boolean);
