...
// ============================================
// RFC-0087: WATER_STORES - No submenu needed
// This widget is specific for water stores/tenants
// ============================================
    ...
    const handle = MyIOLibrary.renderCardComponentHeadOffice(container, {
      ...
      handleActionDashboard: async () => {
        ...
        try {
          if (typeof MyIOLibrary.openDashboardPopupEnergy !== 'function') {
            console.error('[WATER_STORES] [RFC-0072] openDashboardPopupEnergy component not loaded');
          ...
          const modal = MyIOLibrary.openDashboardPopupEnergy({
            ...
            onOpen: (context) => {
              console.log('[WATER_STORES] [RFC-0072] Modal opened:', context);
            },
            onError: (error) => {
              console.error('[WATER_STORES] [RFC-0072] Modal error:', error);
            ...
            onClose: () => {
              ...
              console.log('[WATER_STORES] [RFC-0072] Energy dashboard closed');
            },
          });

          if (!modal) {
            console.error('[WATER_STORES] [RFC-0072] Modal failed to initialize');
            alert('Erro ao abrir dashboard');
            return;
          }

          console.log('[WATER_STORES] [RFC-0072] Energy dashboard opened successfully');
        } catch (err) {
          console.error('[WATER_STORES] [RFC-0072] Error opening energy dashboard:', err);
      ...
      handleActionReport: async () => {
        try {
          ...
          console.warn('[WATER_STORES] Report open blocked:', err?.message || err);
          alert('Credenciais ainda carregando. Tente novamente em instantes.');
        }
      },

      handleActionSettings: async () => {
        console.log('[WATER_STORES] [RFC-0072] Opening settings for store:', entityObject.entityId);

        const jwt = localStorage.getItem('jwt_token');
        if (!jwt) {
          console.error('[WATER_STORES] [RFC-0072] JWT token not found');
        ...
        if (!tbId || tbId === it.ingestionId) {
          LogHelper.warn('[WATER_STORES] Missing/ambiguous TB id for Settings', {
            label: it.label,
            identifier: it.identifier,
            ingestionId: it.ingestionId,
            tbId,
        ...
        try {
          await MyIOLibrary.openDashboardPopupSettings({
            ...
            onSaved: (payload) => {
              LogHelper.log('[WATER_STORES] Settings Saved:', payload);
              showGlobalSuccessModal(6);
            },
            onClose: () => {
              const overlay = document.querySelector('.myio-settings-modal-overlay');
              if (overlay) overlay.remove();
              console.log('[WATER_STORES] [RFC-0072] Settings closed');
            },
          });
        } catch (e) {
          console.error('[WATER_STORES] [RFC-0072] Error opening settings:', e);
        }
      },
    });
  });

  console.log(`[WATER_STORES] Rendered ${visible.length} store cards using renderCardComponentHeadOffice`);
}
...
function bindHeader() {
  $root().on('click', '#btnSearch', () => {
    ...
    if (STATE.searchActive) setTimeout(() => $root().find('#waterStoresSearch').trigger('focus'), 30);
  });

  $root().on('input', '#waterStoresSearch', (ev) => {
...
function setupModalCloseHandlers(modal) {
  ...
  if (applyBtn) {
    applyBtn.addEventListener('click', () => {
      ...
      console.log('[WATER_STORES] [RFC-0072] Filters applied:', {
  ...
  if (resetBtn) {
    resetBtn.addEventListener('click', () => {
      ...
      // Reset UI
      const searchInput = $root().find('#waterStoresSearch')[0];
      ...
      console.log('[WATER_STORES] [RFC-0072] Filters reset');
    });
  }
  ...
  filterTabs.forEach((tab) => {
    tab.addEventListener('click', () => {
      ...
      console.log(
        `[WATER_STORES] Filter tab selected: ${filterType}, checked: ${checkedCount}/${checkboxes.length}`
      );
    });
  });
  ...
  console.log('[WATER_STORES] [RFC-0072] Modal handlers bound (close, apply, reset, filter tabs, search)');
}

function openFilterModal() {
  console.log('[WATER_STORES] [RFC-0072] Opening filter modal...');

  // RFC-0072: Move modal to document.body for full-screen overlay
  let globalContainer = document.getElementById('waterStoresFilterModalGlobal');

  if (!globalContainer) {
    globalContainer = document.createElement('div');
    globalContainer.id = 'waterStoresFilterModalGlobal';
    ...
    if (widgetModal) {
      ...
      globalContainer.innerHTML = `
        <style>
          #waterStoresFilterModalGlobal .shops-modal {
          ...
          #waterStoresFilterModalGlobal .shops-modal.hidden {
            display: none !important;
          }

          #waterStoresFilterModalGlobal .shops-modal-card {
            background: #fff;
            border-radius: 16px;
            width: 90%;
            max-width: 900px;
          ...
          #waterStoresFilterModalGlobal .shops-modal-header {
            display: flex;
            align-items: center;
          ...
          #waterStoresFilterModalGlobal .shops-modal-header h3 {
            margin: 0;
            font-size: 16px;
            font-weight: 700;
            color: #3E1A7D;
          ...
          #waterStoresFilterModalGlobal .shops-modal-body {
            flex: 1;
            overflow-y: auto;
            padding: 20px;
          }

          #waterStoresFilterModalGlobal .shops-modal-footer {
            display: flex;
            gap: 12px;
          ...
          #waterStoresFilterModalGlobal .filter-tabs {
            display: flex;
            flex-wrap: wrap;
            gap: 8px;
            margin-bottom: 16px;
          ...
          #waterStoresFilterModalGlobal .filter-tab {
            display: inline-flex;
            align-items: center;
            gap: 4px;
            padding: 8px 12px;
          ...
          #waterStoresFilterModalGlobal .filter-tab:hover {
            background: #f7fbff;
            border-color: #1f6fb5;
            color: #1f6fb5;
          }

          #waterStoresFilterModalGlobal .filter-tab.active {
          ...
          #waterStoresFilterModalGlobal .filter-search {
            display: flex;
            align-items: center;
            gap: 8px;
            border: 1px solid #D6E1EC;
          ...
          #waterStoresFilterModalGlobal .filter-search input {
            flex: 1;
            border: none;
            outline: none;
            font-size: 13px;
          ...
          #waterStoresFilterModalGlobal .filter-search svg {
            width: 18px;
            height: 18px;
            fill: #6b7a90;
          }

          #waterStoresFilterModalGlobal .filter-search .clear-x {
            border: none;
          ...
          #waterStoresFilterModalGlobal .checklist {
            max-height: 300px;
            overflow-y: auto;
            border: 1px solid #D6E1EC;
            border-radius: 8px;
          ...
          #waterStoresFilterModalGlobal .check-item {
            display: flex;
            align-items: center;
            gap: 8px;
            padding: 8px;
          ...
          #waterStoresFilterModalGlobal .check-item:hover {
            background: #f8f9fa;
          }

          #waterStoresFilterModalGlobal .check-item input[type="checkbox"] {
            width: 18px;
            height: 18px;
          }

          #waterStoresFilterModalGlobal .check-item span {
            font-size: 13px;
            color: #1C2743;
          }

          #waterStoresFilterModalGlobal .btn {
            padding: 10px 16px;
            border: 1px solid #D6E1EC;
            border-radius: 10px;
            font-size: 14px;
          ...
          #waterStoresFilterModalGlobal .btn.primary {
            background: #3E1A7D;
            color: #fff;
            border-color: #3E1A7D;
          }

          #waterStoresFilterModalGlobal .btn.primary:hover {
            background: #2F1460;
          }

          #waterStoresFilterModalGlobal .block-label {
            font-size: 14px;
            font-weight: 600;
            color: #1C2743;
            margin-bottom: 8px;
          ...
          #waterStoresFilterModalGlobal .radio-grid {
            display: grid;
            grid-template-columns: repeat(2, 1fr);
            gap: 8px;
          }

          #waterStoresFilterModalGlobal .radio-grid label {
            display: flex;
            align-items: center;
            gap: 8px;
            padding: 8px;
          ...
          #waterStoresFilterModalGlobal .radio-grid label:hover {
            background: #f8f9fa;
          }

          #waterStoresFilterModalGlobal .muted {
            font-size: 12px;
            color: #6b7a90;
            margin-top: 4px;
          }

          #waterStoresFilterModalGlobal .icon-btn {
            border: 0;
            background: transparent;
            cursor: pointer;
            padding: 4px;
      ...
      console.log('[WATER_STORES] [RFC-0072] Modal moved to document.body with inline styles and handlers');
    } else {
      console.error('[WATER_STORES] [RFC-0072] Filter modal not found in template');
      return;
    }
  }
  ...
  console.log('[WATER_STORES] Filter counts:', counts);

  // Populate device checklist
  const checklist = globalContainer.querySelector('#deviceChecklist');
  if (!checklist) {
    console.error('[WATER_STORES] ❌ deviceChecklist element not found!');
    return;
  }
  ...
  console.log(`[WATER_STORES] Filter modal opened with ${list.length} stores`);
}
function closeFilterModal() {
  // Try to find modal in global container first
  const globalContainer = document.getElementById('waterStoresFilterModalGlobal');
  if (globalContainer) {
    const modal = globalContainer.querySelector('#filterModal');
    if (modal) {
      modal.classList.add('hidden');
      console.log('[WATER_STORES] [RFC-0072] Filter modal closed');
      return;
    }
  }
...
/**
 * Normaliza valor de m³ para MWh com 2 decimais
 * @param {number} m3Value - valor em m³
 * @returns {number} valor em MWh arredondado
 */
function normalizeToMWh(m3Value) {
  if (typeof m3Value !== 'number' || isNaN(m3Value)) return 0;
  return Math.round((m3Value / 1000) * 100) / 100;
}
  ...
  window.addEventListener('myio:telemetry:clear', (ev) => {
    ...
    try {
      ...
      if ($waterStoresList.length > 0) {
        ...
        LogHelper.log(`[TELEMETRY ${WIDGET_DOMAIN}] ✅ waterStoresList cleared`);
      }
...
self.onDestroy = function () {
  ...
  if (globalContainer) {
    ...
    console.log('[WATER_STORES] [RFC-0072] Global modal container removed on destroy');
  }
