...
function initializeSensorCards(sensors) {
  ...
  sensors.forEach((sensor) => {
    ...
    if (typeof MyIOLibrary !== 'undefined' && MyIOLibrary.renderCardComponentHeadOffice) {
      // Tenta obter as constantes de status da lib, ou usa strings padrão como fallback
      const STATUS_ON = window.MyIOLibrary?.DeviceStatusType?.POWER_ON || 'running';
      const STATUS_OFF = window.MyIOLibrary?.DeviceStatusType?.NO_INFO || 'no_info';

      // REGRA SIMPLES: Tem temperatura? Então está Online/Ligado.
      const simpleStatus =
        sensor.temperature !== null && sensor.temperature !== undefined ? STATUS_ON : STATUS_OFF;
      ...
      const entityObject = {
        ...
        // --- [FIX 1] Watchdog ---
        // Informa à lib que a "última conexão" foi agora (data da leitura)
        // Isso impede que ela calcule "tempo excedido" e jogue para offline
        lastConnectTime: sensor.lastUpdate,

        // --- [FIX 2] Status Visual ---
        // Forçamos "Ligado" se tiver dados.
        // Ignoramos 'hot'/'cold' por enquanto conforme solicitado.
        deviceStatus: simpleStatus,
      };

      MyIOLibrary.renderCardComponentHeadOffice(container, {
        entityObject: entityObject,
        // Garante margem de segurança para o watchdog (60 min ou o que vier da config)
