...
self.onInit = function () {
  ...
  */
  ...
  function readFromCtx() {
    ...
    const kpi = {
      avgTemp: Number(
        ctx.$scope?.latestTelemetry?.[s.kpi?.avgTempKey] ??
          ctx.latestData?.[s.kpi?.avgTempAlias]?.data?.[s.kpi?.avgTempKey]?.[0]?.[1] ??
          NaN
      ),
      hvacKw: Number(
        ctx.$scope?.latestTelemetry?.[s.kpi?.hvacKey] ??
          ctx.latestData?.[s.kpi?.hvacAlias]?.data?.[s.kpi?.hvacKey]?.[0]?.[1] ??
          NaN
      ),
      opMinutes: Number(
        ctx.$scope?.latestTelemetry?.[s.kpi?.opMinKey] ??
          ctx.latestData?.[s.kpi?.opMinAlias]?.data?.[s.kpi?.opMinKey]?.[0]?.[1] ??
          NaN
      ),
    };
