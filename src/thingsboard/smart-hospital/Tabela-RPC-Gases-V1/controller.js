...
function getCache(centralId, s, e) {
  const k = cacheKey(centralId, s, e),
    c = gasCache.get(k);
...
self.onInit = function () {
  ...
  s.downloadPDF = () => {
    ...
    const { transformedData, devices } = transformData(s._flatData);
    exportToPDF(transformedData, devices);
  };
  s.downloadCSV = () => {
    if (!s._flatData || s._flatData.length === 0) {
      alert('Sem dados disponíveis para exportar.');
    ...
    const { transformedData, devices } = transformData(s._flatData);
    exportToCSV(transformedData, devices);
  };
