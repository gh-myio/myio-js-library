...
function classifyDevice(labelOrName = '') {
  ...
  if (/bomba|chiller|pista/.test(s)) return 'bomba_chiller';
...
function applyGroupVisibilityAndMap() {
  ...
  self.ctx.isGroupVisible = (name) =>
    !!(self.ctx.groupDivs && self.ctx.groupDivs[name] && self.ctx.groupDivs[name].length);
...
async function openDashboardPopupEnergy(
  ...
  function renderWidget() {
    ...
    const color =
      percentageType === 'increase' ? '#D32F2F' : percentageType === 'decrease' ? '#388E3C' : '#000';
...
`);
  ...
  $popup.off('click.detailLoad', '.detail-load').on('click.detailLoad', '.detail-load', async () => {
    ...
    try {
      ...
      const timeZoneIdentifier =
        self.ctx.timeWindow.timezone || self.ctx.settings.timezone || 'America/Sao_Paulo';

      self.chartInstance = window.EnergyChartSDK.renderTelemetryChart(self.chartContainerElement, {
        version: 'v2',
        clientId: CLIENT_ID,
  ...
  function renderEnergyChartInPopup({ ingestionId, startDateTime, endDateTime, settings }) {
...
`);
  ...
  $popup.find('.btn-salvar').on('click', async () => {
    ...
    try {
      ...
      if (!entityResponse.ok) throw new Error('Erro ao buscar entidade para atualizar label');
      ...
      if (!updateLabelResponse.ok) throw new Error('Erro ao atualizar etiqueta (label)');

      // 3. Enviar os atributos ao SERVER_SCOPE
      const attrResponse = await fetch(`/api/plugins/telemetry/DEVICE/${entityId}/SERVER_SCOPE`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'X-Authorization': `Bearer ${jwtToken}`,
    ...
    if (device.isValid) {
      $row = $(`
                <tr>
                    ...
                    <td>${deviceId}</td>
                    <td>${
                      device.consumptionKwh != null ? MyIOLibrary.formatEnergy(device.consumptionKwh) : '-'
                    }</td>
                </tr>
            `);
    } else {
...
function exportToCSVAll(reportData) {
  ...
  link.setAttribute(
    'download',
    `relatorio_consumo_geral_por_loja_${new Date().toISOString().slice(0, 10)}.csv`
  );
...
`);
  ...
  // Local inputs (scoped to this popup)
  $popup.off('change.allDates', '#startDate,#endDate').on('change.allDates', '#startDate,#endDate', () => {
    setAllDates({
  ...
  const attributeFetchPromises = datasources.map(async (ds) => {
    const entityLabel =
      ds.label || ds.entityLabel || ds.entityName || `Dispositivo (ID: ${ds.entityId.substring(0, 5)})`;
    ...
    try {
      ...
      const deviceAttributes = await attributeService
        .getEntityAttributes({ id: ds.entityId, entityType: ds.entityType }, 'SERVER_SCOPE', [
          'centralId',
          'slaveId',
          'deviceId',
          'ingestionId',
      ...
      const slaveIdValue =
        typeof slaveIdRawValue === 'string' ? parseInt(slaveIdRawValue, 10) : slaveIdRawValue;
  ...
  $('#loadDataBtn').on('click', async () => {
    ...
    try {
      ...
      console.log(
        `[loadDataBtn] Calling Data API customer totals: ${baseUrl.split('?')[0]} with customerId=${customerId}`
      );
...
function exportToCSV(reportData, entityLabel, totalconsumption, entityUpdatedIdentifiers, insueDate) {
  ...
  link.setAttribute(
    'download',
    `relatorio_consumo_${new Date().toISOString().slice(0, 10)}_${entityLabel}.csv`
  );
...
function updateInfoCardsAndChart(groupSums, items) {
  ...
  if (delta >= 0) {
    ...
    labels = ['Chiller, Bombas, Área Comum', Object.keys(GROUPS)[2], 'Área Comum (-)'];
    console.warn(
      `[delta NOK] ⚠️ delta: "${delta}" | Entrada Total: "${entradaTotal}" | Consumo Total: "${consumoTotal}" (Lojas "${lojasVal}", Admin "${(admin1 + admin2) * 1000}" e Bombas "${adminVal * 1000}")`
    );
  }
  ...
  $infoList.append(createInfoCard(areaTitle, areaValue, percInterno(areaValue), areaIcon));
  ...
  */
  ...
  if (!hideCommonArea) {
    ctx.areaChart = new Chart(document.getElementById('areaChart').getContext('2d'), {
      type: 'pie',
      data: {
        labels,
        datasets: [
          {
            ...
            backgroundColor:
              delta >= 0 ? ['#2196f3', '#4caf50', '#ff9800'] : ['#2196f3', '#4caf50', '#f44336'], // vermelho para “Ajuste”
          },
        ],
      },
      options: {
...
self.onInit = async function () {
  ...
  // Abertura de popups
  ctx.$container.off('click', '.device-card-centered').on('click', '.device-card-centered', function () {
    const entityId = $(this).data('entity-id');
    const entityType = $(this).data('entity-type');

    ctx.stateController.openState('default', { entityId: { id: entityId, entityType } }, false);
  });

  // Ações (dashboard, report, config)
  ctx.$container.off('click', '.card-action').on('click', '.card-action', function (e) {
    e.stopPropagation();
  ...
  // Checkbox de seleção
  ctx.$container.off('click', '.checkbox-icon').on('click', '.checkbox-icon', function (e) {
    e.stopPropagation();
  ...
  if (self.startDate === undefined) self.startDate = formatDateWithTimezoneOffset(firstDay);

  if (self.endDate === undefined) self.endDate = formatDateWithTimezoneOffset(now);

  // Inicializa datas para exibição no input date (yyyy-MM-dd)
  if (!self.startDateFormatted) self.startDateFormatted = formatDateForInput(new Date(self.startDate));

  if (!self.endDateFormatted) self.endDateFormatted = formatDateForInput(new Date(self.endDate));
  ...
  try {
    ...
    console.log(
      `[onInit] Successfully mapped ${Object.values(entityMap).filter((item) => item.isValid).length} devices using ingestionId`
    );
  } catch (err) {
  ...
  for (const group in groupSums) {
    ...
    ctx.groupDivs[group].find(`[data-group="${dataAttr}"]`).text(MyIOLibrary.formatEnergy(groupSums[group]));
  }
...
function getValueByDatakey(dataList, dataSourceNameTarget, dataKeyTarget) {
  for (const item of dataList) {
    if (item.datasource.name === dataSourceNameTarget && item.dataKey.name === dataKeyTarget) {
      const itemValue = item.data?.[0]?.[1];

      if (itemValue !== undefined && itemValue !== null) {
        return itemValue;
      } else {
        console.warn(`Valor não encontrado para ${dataSourceNameTarget} - ${dataKeyTarget}`);
        return null;
      }
    }
  }
