...
let  DEFAULT_CUSTOMER_ID = " "; // e.g., "73d4c75d-c311-4e98-a852-10a2231007c4"
let CLIENT_ID = " ";
let CLIENT_SECRET = " ";
...
function resolveTimeZone(tzCandidate) {
  if (typeof tzCandidate === "string" && tzCandidate.trim()) return tzCandidate.trim();
  try {
...
function getUiDateRange() {
  ...
  const toUTC0 = s => new Date(`${String(s).slice(0,10)}T00:00:00Z`).getTime();
  if (toUTC0(start) > toUTC0(end)) { const t = start; start = end; end = t; }
...
function looksLikeTankLabel(text) {
  const t = norm(text);
  return /(caixa|reservatorio|cisterna|superior|inferior|nivel[_ ]?terraco|torre)/.test(t);
}
...
function keysFromDeviceAttrs(attrs = {}) {
  const central = attrs.centralId || attrs.gatewayId || attrs.ingestionId || attrs.deviceCentralId;
...
function matchApiRow(deviceAttrs, apiItem, flags = FLAGS) {
  if (flags.STRICT_MATCH_BY_INGESTION_ID) {
    ...
    if (deviceIngestion && apiIngestion && deviceIngestion === apiIngestion) {
      ...
      if (deviceSlave != null && apiSlave != null && Number(deviceSlave) === Number(apiSlave)) {
    ...
    for (const deviceKey of deviceKeys) {
      for (const apiKey of apiKeys) {
        if (deviceKey === apiKey) {
          return { matched: true, confidence: 'medium', method: 'key_match', key: deviceKey };
        }
      }
    }
    ...
    for (const key of keys) {
      if (key) {
        ...
        if (flags.VERBOSE_LOGS) {
          logInfo(`[buildTotalsMapFromApi] Mapped key "${key}" -> ${total} (${deviceInfo.name})`);
        }
      }
    }
  }
  
  logInfo(`[buildTotalsMapFromApi] Created ${totalsMap.size} key mappings from ${apiList.length} API items`);
  return totalsMap;
}
...
async function fetchCustomerWaterTotals(customerId, { startTime, endTime } = {}) {
  ...
  try {
    ...
    const normalizedResponse = {
      data: Array.isArray(json) ? json : (Array.isArray(json?.data) ? json.data : []),
      summary: json?.summary || { 
        totalDevices: Array.isArray(json) ? json.length : (json?.data?.length || 0), 
        totalValue: 0 
      }
    };
    ...
    if (!json?.summary?.totalValue && normalizedResponse.data.length > 0) {
      normalizedResponse.summary.totalValue = normalizedResponse.data.reduce((sum, item) => {
    ...
    <div class="device-data-row">
        <div style="display: flex; align-items: center; gap: 6px; font-size: 0.85rem; font-weight: bold; color: #28a745;">
          <span class="flash-icon flash">⚡</span>
                  <span class="consumption-value">${MyIOLibrary.formatEnergyByGroup(value, "Caixas D'Água")}</span>
          ${
            percentage != null
    ...
    return `
            <div class="myio-sum-comparison-card" style="
                ...
                <div style="text-align:center; font-size:0.85rem; color:#757575; margin-bottom:12px; display:none">
                    Último período: 
                    <strong> ${MyIOLibrary.formatEnergyByGroup(lastConsumption, "Lojas")}</strong>
                </div>
...
async function openDashboardPopupWater(
  ...
  const gatewayId = entityCentralId;
  const apiBaseUrl = settings.apiBaseUrl || "https://ingestion.myio-bas.com";
  const timezone = self.ctx.timeWindow.timezone || self.ctx.settings.timezone || "America/Sao_Paulo";
  ...
  function renderWidget() {
    ...
    const heigtconsumption = MyIOLibrary.formatEnergyByGroup(entityComsuption * 100, "Caixas D'Água");
...
<svg xmlns="http://www.w3.org/2000/svg" width="32px" height="32px" viewBox="0 -880 960 960" style="display:block;">
  ...
  <ellipse cx="420" cy="-500" rx="38" ry="62" fill="rgba(255,255,255,0.4)" transform="rotate(-25 420 -500)"/>
    ...
    <!-- Último período -->
    <div style="text-align:center; font-size:0.85rem; color:#757575; margin-bottom:12px; display:none">
        Porcentagem da Caixa: <strong>${MyIOLibrary.formatNumberReadable(percent)}%</strong>
    </div>
  ...
  try {
    ...
    document.getElementById("chartTitle").innerText = `Nível de Água - ${labelDefault}`;
  } catch (err) {
...
`);
  ...
  const datasources = originalDatasources.filter(ds => ds.aliasName !== "Caixas de agua");
  ...
  const attributeFetchPromises = datasources.map(async (ds) => {
    const entityLabel = ds.label || ds.entityLabel || ds.entityName || `Dispositivo (ID: ${ds.entityId.substring(0, 5)})`;
  ...
  $("#loadDataBtn").on("click", async () => {
    ...
    try {
      ...
      const apiData = await fetchCustomerWaterTotals(customerId, { startTime, endTime });
      ...
      self.ctx.$scope.reportData.forEach((device) => {
        if (device.ingestionId) {
          ...
          if (apiDevice) {
            device.consumptionM3 = Number(apiDevice.totalConsumption || apiDevice.total_value || 0);
          } else {
      ...
      // Set default sorting
      self.ctx.$scope.sortColumn = self.ctx.$scope.sortColumn || 'consumptionM3';
  ...
  function renderHeaderStats(reportData) {
    ...
    const totalConsumption = validDevices.reduce((sum, d) => sum + (d.consumptionM3 || 0), 0);
...
function openWaterChartIframe(deviceId) {
  ...
  if (!container) {
    console.error("openWaterChartIframe(): Neither #chart-wrapper nor #chart-container found");
    return;
  }
  ...
  */
  const { startISO, endISO } = getUiDateRange();
  ...
  const iframeBase   = settings.iframeBaseUrl || "https://graphs.apps.myio-bas.com";
  const theme        = settings.theme || "light";

  console.log(`Initializing v2 water chart: deviceId=${deviceId}, start=${startISO}, end=${endISO}, granularity=${gran}, tz=${timezone}`);
  ...
  if (self.chartInstance && typeof self.chartInstance.on === "function") {
    self.chartInstance.on("drilldown", (evt) => console.log("v2 Water SDK Drilldown:", evt));
    self.chartInstance.on("error", (err) => {
      console.error("v2 Water SDK Error:", err);
      container.innerHTML = `<div style="padding: 20px; text-align: center; color: red;">v2 Water Chart Error: ${err.message || "Unknown error"}</div>`;
    });
  }
}
...
`);
  ...
  console.log('[WATER REPORT] popup open', { deviceId: entityId, ingestionId: entityIngestionId });
  ...
  function updateTable() {
    ...
    sortedData.forEach((item, index) => {
      ...
      tr.innerHTML = `
        ...
        <td style="padding: 8px 12px; color: ${corTexto}; background-color: ${corFundo};text-shadow: 0 2px 4px rgba(0, 0, 0, 0.5)">${MyIOLibrary.formatEnergyByGroup(item.consumptionM3, "Ambientes")}</td>
    `;
  ...
  $popup.off('click.reportLoad', '.report-load, #btn-load')
    .on('click.reportLoad', '.report-load, #btn-load', async () => {
      ...
      try {
        ...
        console.log(`[WATER REPORT] Fetching data for ingestionId=${entityIngestionId} from ${startTime} to ${endTime}`);
        
        // Build Data API URL with required parameters for water
        const url = `${DATA_API_HOST}/api/v1/telemetry/devices/${entityIngestionId}/water?startTime=${encodeURIComponent(startTime)}&endTime=${encodeURIComponent(endTime)}&granularity=1d&page=1&pageSize=1000&deep=0`;

        console.log(`[WATER REPORT] Calling Data API: ${url.split('?')[0]} with deviceId=${entityIngestionId}`);
        ...
        if (!Array.isArray(data) || data.length === 0) {
          console.warn("[WATER REPORT] Data API returned empty or invalid response");
        ...
        document.getElementById("total-consumo").textContent = MyIOLibrary.formatEnergyByGroup(totalconsumption, "Ambientes");
        document.getElementById("inssueDate").textContent = insueDate;

        updateTable();
        habilitarBotaoExport();
        
        console.log(`[WATER REPORT] Successfully processed ${consumption.length} consumption records, total: ${totalconsumption} m³`);
...
self.onInit = async function () {
  const ctx = self.ctx;
  DEFAULT_CUSTOMER_ID = self.ctx.settings.DEFAULT_CUSTOMER_ID||" "; // e.g., "73d4c75d-c311-4e98-a852-10a2231007c4"
  CLIENT_ID = self.ctx.settings.clientID||" ";
  CLIENT_SECRET = self.ctx.settings.secretid ||" ";
  console.log("[water/controller] onInit() | self.ctx >>> ", self.ctx);
  console.log("DEFAULT_CUSTOMER_ID",DEFAULT_CUSTOMER_ID)
  ...
  if (ctx.groupDivs["Caixas D'Água"].length === 0 || ctx.groupDivs["Ambientes"].length === 0) {
    setTimeout(() => {
  ...
  ctx.$container
    ...
    const entityConsumption = (consumptionAttr !== undefined && consumptionAttr !== null && !Number.isNaN(Number(consumptionAttr)))
      ? Number(consumptionAttr)
      : (function parseLocalizedNumber(txt) {
          // ex: "12.345,67 kWh" -> 12345.67
          const onlyNums = String(txt).replace(/[^\d,.-]/g, "")
      ...
      console.log(`[card action] action=${action} entityId=${entityId} label=${entityLabel} consumption=${entityConsumption} (${$span.text()})`);

      // Context: is this a water-tank card?
      const isWaterCard = $card.closest(".group-card").hasClass("caixa");

      if (action === "dashboard") { // Caixa d'água
        if (isWaterCard) {
          // Tanks use the canvas-based water popup
          return openDashboardPopupWater(
            entityId, entityType, entitySlaveId, entityCentralId,
            entityLabel, entityConsumption, percent
          );
        }

        // Ambientes: open the “hidro” popup; if we have ingestionId, render the SDK/iframe
        await openDashboardPopupHidro(
          entityId, entityType, entitySlaveId, entityCentralId, entityLabel, entityConsumption
        );

        if (entityIngestionId) {
          setTimeout(() => openWaterChartIframe(entityIngestionId), 100);
        }
        return;
      }

      if (action === "dashboard_water") { // Hidrometro / Ambientes
        return openDashboardPopupWater(
          entityId, entityType, entitySlaveId, entityCentralId,
          entityLabel, entityConsumption, percent
        );
      ...
      if (action === "report") {
        return openDashboardPopupReport(
          entityId, entityType, entityIngestionId, entityLabel,
          entityConsumption, entityUpdatedIdentifiers, sourceName
        );
      }
  ...
  (function fixTopbarOffset() {
    try {
      ...
      const extra = parseFloat(cs.marginTop || 0) + parseFloat(cs.marginBottom || 0);
...
function classify(text) {
  ...
  if (/(caixa|reservat[óo]rio|cisterna|superior|inferior|nível[_ ]?terraço|nivel[_ ]?terraco|torre)/.test(v)) {
...
self.onDataUpdated = async function () {
  ...
  for (const g in ctx.groups) {
    ...
    if (ctx.groupDivs[g] && ctx.groupDivs[g].length > 0) {
      ...
      console.error(`[water/controller] onDataUpdated() ERROR: Could not find or initialize group div for "${g}"`);
    }
  }
  ...
  logInfo(`[water/controller] onDataUpdated() mapping datasource devices, total devices: ${devices.length}`);
  ...
  const dsDevices = devices.map((device) => {
    ...
    const percent = getValueByDatakey(ctx.data, sourceName, "water_percentage") || 0;
    const level = getValueByDatakey(ctx.data, sourceName, "water_level") || 0;

    logInfo(`[water/controller] Datasource device: ${labelOrName} -> Group: ${group} (datasource: ${device.aliasName || device.name})`);
  ...
  try {
    ...
    const customerId = DEFAULT_CUSTOMER_ID || "73d4c75d-c311-4e98-a852-10a2231007c4";
   ...
   // console.log("[water/controller] onDataUpdated() totalsByKey map size:", totalsByKey.size);
    ...
    if (FLAGS.INJECT_API_ONLY_DEVICES) {
      logInfo(`[water/controller] onDataUpdated() injecting API-only devices, checking ${apiList.length} API items`);
      ...
      for (const item of apiList) {
        ...
        if (alreadyInDatasource) {
          logInfo(`[water/controller] API device already in datasources: ${item.name || item.id} (keys: ${probeKeys.join(', ')})`);
          continue;
        }
        ...
        logInfo(`[water/controller] INJECTING API-only device: ${deviceLabel} (${syntheticEntityId})`);
    ...
    logInfo(`[water/controller] devices summary: datasource=${dsDevices.length}, injected=${injectedDevices.length}, total=${dsDevices.length + injectedDevices.length}`);
    ...
    for (const dev of Object.values(entityMap)) {
      ...
      if (!matched) {
        dev.val = 0;
        if (dev.source === 'api') {
          console.warn(`[water/controller] No total found for injected API device "${dev.label}" with keys: [${keys.join(', ')}]`);
        }
      }
    }
  ...
  console.log("[water/controller] onDataUpdated() calculating group sums from matched devices, total items:", items.length);
...
//   });
    ...
    ({
            ...
            <div style="display: flex; flex-direction: column; justify-content: center; align-items: center; height: 100%; width: 85%">
              ...
              <div class="device-data-row">
                <div class="consumption-main">
                  ...
                  <span class="device-title-percent">(${MyIOLibrary.formatNumberReadable(percent)}%)</span>
                </div>
              </div>
            </div>
          </div>
     ...
     // console.log(`[water/controller] Creating/updating card for device "${labelOrName}" in group "${group}" with value ${adjustedVal}`);
      ...
      //  console.log(`[water/controller] Updating existing card for "${labelOrName}" in group "${group}"`);
        $existingCard.find(".consumption-value").text(MyIOLibrary.formatEnergyByGroup(adjustedVal, group));
       ...
       // console.log(`[water/controller] Group div exists for "${group}":`, ctx.groupDivs[group] && ctx.groupDivs[group].length > 0);
        ...
        //  console.log(`[water/controller] Successfully added card to "${group}" group. New count: ${ctx.groups[group].length}`);
          ...
          console.error(`[water/controller] ERROR: Group div not found for "${group}". Available groups:`, Object.keys(ctx.groupDivs));
        }
      }
    }
  );
  ...
  for (const group in groupSums) {
    ...
    if (ctx.groupDivs[group] && ctx.groupDivs[group].length > 0) {
      ...
      ctx.groupDivs[group].find(`[data-group="${group}"]`).text(MyIOLibrary.formatEnergyByGroup(groupSums[group], group));
  ...
  logInfo(`[water/controller] Total devices rendered: ${totalDevices} (was 27, target was 93)`);
  logInfo(`[water/controller] Ambientes: ${ambientesCount} devices, ${groupSums["Ambientes"]} m³`);
  logInfo(`[water/controller] Caixas D'Água: ${caixasCount} devices, ${groupSums["Caixas D'Água"]} m³`);
