    ...
    if (btnAnnotationNotif) {
      ...
      btnAnnotationNotif.addEventListener('click', (e) => {
        ...
        if (!_annotationFilterActive) {
          ...
          if (total === 0) {
            LogHelper.log('[HEADER] annotation filter suppressed — no annotations in customer scope');
            return;
          }
        }
        ...
        window.dispatchEvent(new CustomEvent('myio:global-annotation-filter', { detail: { mode } }));
