...
function _itemHasActiveAnnotation(item) {
  ...
  return arr.some((a) => a && typeof a === 'object' && a.status !== 'archived');
}
...
const _QF_GROUPS = [
  ...
  },
  {
    label: 'Consumo',
    filters: [
      ['withConsumption', 'Com consumo'],
      ['noConsumption', 'Sem consumo'],
    ],
  },
  {
    label: 'Tipo',
...
function _showQuickFilterDevices(triggerEl, filterId) {
  ...
  const devices = (STATE.itemsBase || []).filter((it) => (it._qfTags || []).indexOf(filterId) !== -1);
  ...
  const dotColors = {
    ...
    alert: '#f59e0b',
    failure: '#ef4444',
    withConsumption: '#15803d',
    noConsumption: '#6b7280',
    stores: '#9333ea',
    hvac: '#0891b2',
    elevators: '#7c3aed',
    escalators: '#db2777',
...
function _sortChecklistDom($m) {
  ...
  nodes.sort((a, b) => {
    ...
    if (mode === 'alpha_desc') return labelOf(b).localeCompare(labelOf(a), 'pt-BR', { sensitivity: 'base' });
...
function _renderFilterStats($m, list) {
  ...
  const near3 = [...items].sort((a, b) => Math.abs(a.v - avg) - Math.abs(b.v - avg)).slice(0, 3);
...
async function _handleSyncGCDR() {
  ...
  const defaultAssetId = Object.keys(_assetFreq).sort((x, y) => _assetFreq[y] - _assetFreq[x])[0] || '';
  if (!defaultAssetId) {
    LogHelper.warn(
