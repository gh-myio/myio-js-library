...
Object.assign(window.MyIOUtils, {
  ...
  updateEquipmentStats: (devices, _cache) => {
    LogHelper.log(
  ...
  getConsumptionRangesHierarchical: async (_deviceId, _deviceType, _limits, _metric, _fallback) => {
...
function createFilterModal(config) {
  ...
  const {
    ...
    onApply = () => {},
    // eslint-disable-next-line no-unused-vars -- accepted config option; reset handler not yet wired
    onReset = () => {},
    onClose = () => {},
  } = config;
...
function calculateDeviceStatusMasterRules(options = {}) {
  const {
    ...
    // eslint-disable-next-line no-unused-vars -- accepted option; not consumed in this path
    domain = 'energy',
  } = options;
