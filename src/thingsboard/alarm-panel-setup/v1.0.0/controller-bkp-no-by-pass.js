...
function render() {
  ...
  if (state.settings.appearance.showHeader) {
    ...
    html +=
      '<button class="ap-btn ap-btn-export" onclick="AlarmPanel.openExportModal()" title="Exportar PDF">&#128196; PDF</button>';
...
function renderCustomersBar() {
  ...
  if (state.customers.length <= 1) {
    ...
    if (state.customers.length === 1) {
      ...
      var deviceCount = state.devices.filter(function (d) {
        return d.customerId === c.id;
      }).length;
      html = '<div class="ap-customers-bar">';
  ...
  state.customers.forEach(function (c) {
    ...
    html +=
      '<div class="ap-customer-pill selectable' +
      (isSelected ? ' selected' : '') +
      '" onclick="AlarmPanel.toggleCustomer(\'' +
      c.id +
      '\')">';
    html += '<span class="ap-customer-name">' + escapeHtml(c.name) + '</span>';
...
function renderDevices() {
  ...
  html +=
    '<input type="text" id="ap-device-search" class="ap-search-input" placeholder="Buscar devices..." value="' +
    escapeHtml(state.filters.searchDeviceText || '') +
    '" onkeyup="AlarmPanel.onDeviceSearchChangeGrid(event)" />';
  html += '</div>';

  // Device Filter Button (opens modal)
  var deviceFilterCount = state.filters.selectedDeviceIds.length;
  html +=
    '<button class="ap-filter-btn' +
    (deviceFilterCount > 0 ? ' has-filters' : '') +
    '" onclick="AlarmPanel.openDeviceFilterModal()">';
  ...
  var devicesWithAlarms = allDevices.filter(function (d) {
    return d.activeAlarmCount > 0;
  }).length;
  html +=
    '<label class="ap-dfm-toggle' +
    (state.deviceFilterModalShowOnlyWithAlarms ? ' active' : '') +
    '" style="margin-left: 8px;">';
  html +=
    '<input type="checkbox" ' +
    (state.deviceFilterModalShowOnlyWithAlarms ? 'checked' : '') +
    ' onchange="AlarmPanel.toggleDeviceFilterModalOnlyAlarms()" />';
  ...
  if (devices.length === 0) {
    html +=
      '<div class="ap-empty">Nenhum device encontrado. Selecione profiles acima ou ajuste os filtros.</div>';
    return html;
  }
...
function renderSortableHeader(column, label, currentColumn, currentDirection) {
  ...
  return (
    '<div class="ap-grid-header-cell' +
    sortClass +
    '" onclick="AlarmPanel.sortDevices(\'' +
    column +
    '\')">' +
    '<span>' +
    label +
    '</span>' +
    '<span class="ap-sort-icon">' +
    arrow +
    '</span>' +
    '</div>'
  );
...
function renderAlarms() {
  ...
  html += '<div class="ap-search-box">';
  html +=
    '<input type="text" id="ap-alarm-search" class="ap-search-input" placeholder="Buscar alarmes..." value="' +
    escapeHtml(state.filters.searchText) +
    '" onkeyup="AlarmPanel.onSearchChange(event)" />';
  html += '</div>';
  ...
  ['CRITICAL', 'MAJOR', 'MINOR', 'WARNING'].forEach(function (sev) {
    html +=
      '<label><input type="checkbox" ' +
      (state.filters.severities[sev] ? 'checked' : '') +
      ' onchange="AlarmPanel.toggleSeverityFilter(\'' +
      sev +
      '\')"> <span class="ap-badge ' +
...
function renderDeviceFilterModal() {
  ...
  var devicesWithAlarms = devices.filter(function (d) {
    return d.activeAlarmCount > 0;
  }).length;
  ...
  html +=
    '<div class="ap-dfm-title"><span class="ap-dfm-title-icon">&#128270;</span> Filtrar por Devices</div>';
  ...
  html +=
    '<input type="text" id="ap-dfm-search" placeholder="Buscar por nome ou label..." value="' +
    escapeHtml(state.deviceFilterModalSearch) +
    '" ';
  ...
  html +=
    '<div class="ap-dfm-stat">Filtrados: <span class="ap-dfm-stat-value">' +
    filteredDevices.length +
    '</span></div>';
  html +=
    '<div class="ap-dfm-stat">Selecionados: <span class="ap-dfm-stat-value">' +
    selectedCount +
    '</span></div>';
  html +=
    '<div class="ap-dfm-stat">Com alarmes: <span class="ap-dfm-stat-value" style="color: #c62828;">' +
    devicesWithAlarms +
    '</span></div>';
  html += '</div>';
  ...
  if (filteredDevices.length === 0) {
    ...
    sortedGroups.forEach(function (group) {
      ...
      html +=
        '<span class="ap-dfm-group-action" onclick="event.stopPropagation(); AlarmPanel.selectDeviceGroup(\'' +
        group.key +
        '\')">Selecionar</span>';
      html +=
        '<span class="ap-dfm-group-action" onclick="event.stopPropagation(); AlarmPanel.deselectDeviceGroup(\'' +
        group.key +
        '\')">Desmarcar</span>';
      html += '</div>';
      html += '</div>';
      ...
      group.devices.forEach(function (d) {
        ...
        html +=
          '<div class="ap-dfm-item' +
          (isSelected ? ' selected' : '') +
          '" onclick="AlarmPanel.toggleDeviceSelection(\'' +
          d.id +
          '\')">';
        ...
        if (d.currentAlarmSeverity) {
          html +=
            '<span class="ap-dfm-item-badge ' +
            getSeverityClass(d.currentAlarmSeverity) +
            '">' +
            d.currentAlarmSeverity +
            '</span>';
        }
  ...
  if (selectedCount > 0) {
    html +=
      '<button class="ap-dfm-btn ap-dfm-btn-clear" onclick="AlarmPanel.clearAllDevices()">Limpar Seleção</button>';
  }
  html +=
    '<button class="ap-dfm-btn ap-dfm-btn-secondary" onclick="AlarmPanel.selectAllDevicesInModal()">Selecionar Todos</button>';
  html +=
    '<button class="ap-dfm-btn ap-dfm-btn-primary" onclick="AlarmPanel.closeDeviceFilterModal()">Aplicar</button>';
  html += '</div>';
  html += '</div>';
...
function renderFilterModal() {
  ...
  html +=
    '<div class="ap-filter-section-title">Devices <span class="ap-select-all" onclick="AlarmPanel.selectAllDevices()">Selecionar todos</span> <span class="ap-select-all" onclick="AlarmPanel.clearAllDevices()">Limpar</span></div>';
  ...
  filteredDevices.forEach(function (d) {
    ...
    html +=
      '<input type="checkbox" ' + checked + ' onchange="AlarmPanel.toggleDeviceFilter(\'' + d.id + '\')" />';
  ...
  ['CRITICAL', 'MAJOR', 'MINOR', 'WARNING'].forEach(function (sev) {
    ...
    html += '<label class="ap-checkbox-item">';
    html +=
      '<input type="checkbox" ' + checked + ' onchange="AlarmPanel.toggleSeverityFilter(\'' + sev + '\')" />';
  ...
  html +=
    '<button class="ap-btn ap-btn-secondary" onclick="AlarmPanel.resetFilters()" style="margin-right: 8px;">Resetar</button>';
...
function renderExportModal() {
  ...
  if (state.exportGenerating) {
    ...
    html +=
      '<label class="ap-export-option' +
      (state.exportOptions.deviceProfiles ? ' selected' : '') +
      '" onclick="AlarmPanel.toggleExportOption(\'deviceProfiles\')">';
    ...
    html +=
      '<div class="ap-export-option-desc">Inclui: perfis, regras de criação/limpeza, agendamentos</div>';
    html += '</div></label>';

    // Device Map option
    html +=
      '<label class="ap-export-option' +
      (state.exportOptions.deviceMap ? ' selected' : '') +
      '" onclick="AlarmPanel.toggleExportOption(\'deviceMap\')">';
    ...
    html +=
      '<label class="ap-export-option' +
      (state.exportOptions.alarmList ? ' selected' : '') +
      '" onclick="AlarmPanel.toggleExportOption(\'alarmList\')">';
    html += '<input type="checkbox" ' + (state.exportOptions.alarmList ? 'checked' : '') + ' />';
    html += '<div class="ap-export-option-content">';
    html += '<div class="ap-export-option-title">Lista de Alarmes</div>';
    html +=
      '<div class="ap-export-option-desc">Alarmes conforme filtros atuais (' +
  ...
  if (!state.exportGenerating) {
    var hasSelection =
      state.exportOptions.deviceProfiles || state.exportOptions.deviceMap || state.exportOptions.alarmList;
    html +=
      '<button class="ap-btn ap-btn-secondary" onclick="AlarmPanel.closeExportModal()" style="margin-right: 8px;">Cancelar</button>';
    html +=
      '<button class="ap-btn ap-btn-export" onclick="AlarmPanel.generatePDF()"' +
...
function generatePDFDocument() {
    ...
    state.profiles.forEach(function (profile, idx) {
      ...
      doc.text(
        'Devices: ' +
          profile.deviceCount +
          ' | Regras: ' +
          (profile.alarmRules ? profile.alarmRules.length : 0),
        margin + 5,
        yPos
      );
      yPos += 6;
      ...
      if (profile.alarmRules && profile.alarmRules.length > 0) {
        profile.alarmRules.forEach(function (rule, ruleIdx) {
          ...
          if (rule.clearRule && rule.clearRule.condition) {
            doc.text(
              '    Clear: ' + formatCondition(rule.clearRule.condition).substring(0, 80),
              margin + 5,
              yPos
            );
          } else {
  ...
  if (state.exportOptions.deviceMap && state.devices.length > 0) {
    ...
    var deviceData = getFilteredDevices().map(function (d) {
      var profileName = state.profiles.find(function (p) {
        return p.id === d.deviceProfileId;
      });
      return [
        d.name || '-',
        d.label || '-',
...
function renderModal() {
  ...
  html +=
    '<div class="ap-alarm-rules-header"><strong>Regras de Alarme:</strong> ' +
...
function formatSchedule(schedule) {
  ...
  if (schedule.type === 'SPECIFIC_TIME') {
    return (
      'Horário específico: ' +
      formatTimeFromMs(schedule.startsOn) +
      ' às ' +
      formatTimeFromMs(schedule.endsOn)
    );
  }
  ...
  alarmRules.forEach(function (rule, ruleIdx) {
    ...
    if (rule.createRules && Object.keys(rule.createRules).length > 0) {
      ...
      severities.forEach(function (severity, sevIdx) {
        ...
        html +=
          '<div class="ap-alarm-rule-severity">' +
          subNum +
          ' - Setup de Criação: <span class="ap-badge ' +
        ...
        if (createRule.condition) {
          html +=
            '<div class="ap-alarm-rule-condition">' +
            subNum +
            '.1 - REGRA: ' +
            escapeHtml(formatCondition(createRule.condition)) +
            '</div>';
          ...
          if (createRule.condition.condition && createRule.condition.condition[0]) {
            ...
            if (
              firstCond.predicate &&
              firstCond.predicate.value &&
              firstCond.predicate.value.defaultValue !== null
            ) {
              html +=
                '<div class="ap-alarm-rule-default">' +
                subNum +
                '.2 - Valor padrão: ' +
                firstCond.predicate.value.defaultValue +
                '</div>';
            }
          }
        }
        ...
        // Schedule
        html +=
          '<div class="ap-alarm-rule-schedule">' +
          subNum +
          '.' +
          (createRule.condition ? '3' : '2') +
          ' - ' +
          escapeHtml(formatSchedule(createRule.schedule)) +
          '</div>';
    ...
    .then(function (alarms) {
      ...
      filteredDevices.forEach(function (device) {
        ...
        if (activeAlarms.length > 0) {
          var hasAck = activeAlarms.some(function (a) {
