...
function mountAmbientesPanel(host, settings, assetAmbientHierarchy) {
  ...
  var panel = new MyIOLibrary.CardGridPanel({
    ...
    gridRowGap: settings.cardGridRowGap || '6px',
    singleColumn: true,
    maxCardWidth: settings.ambienteCardMaxWidth || '280px',
...
function openBASWaterModal(device, _settings) {
  ...
  var startDateStr = toLocalISODate(new Date(_waterDateRange.startTs));
  var endDateStr   = toLocalISODate(new Date(_waterDateRange.endTs));
...
function mountEnergyPanel(host, settings, classified) {
  ...
  var panel = new MyIOLibrary.CardGridPanel({
    ...
    gridRowGap: settings.cardGridRowGap || '6px',
    singleColumn: true,
    maxCardWidth: settings.energyCardMaxWidth || '280px',
...
async function fetchIngestionData(domain, customerId, clientId, clientSecret, period, startTs, _endTs) {
  ...
  // Day boundaries use local midnight so they match the date the user selected
  var dayBoundaries = [];
