// Controller for blinking-air-list-with-consumption-and-temperature-v3
/* eslint-disable */
/*
...
const DEBUG = false;
let MinConsumption = 15;
let MinConsumptionSplit = 15;
...
self.onInit = function () {
    ...
    Object.entries(grouped).forEach(([label, group]) => {
      ...
      const isOnline = group.connectionStatus === true;
    ...
    ctx.$container.on('click', '.device-card-centered', function (e) {
      ...
      getParentAssetViaHttp(entityPayload)
          .then(assetRef => {
            const state = [{
              id: 'default',
              params: {
    ...
    const MinConsumptionCheck = (() => {
        const val = parseInt(ctx.settings && ctx.settings.minConsumption, 10);
        return !isNaN(val) && val > 0 ? val : 15;
    })();
    
    const MinConsumptionSplitCheck = (() => {
        const val = parseInt(ctx.settings && ctx.settings.minConsumptionSplit, 10);
        return !isNaN(val) && val > 0 ? val : 15;
    })();    
    
    MinConsumption = MinConsumptionCheck;
    MinConsumptionSplit = MinConsumptionSplitCheck;
};
...
self.onDataUpdated = function () {
  ...
  ctx.data.forEach(ds => {
    ...
    if (key === 'consumption') {
      const isOn = val > (isSalaOnline ? MinConsumptionSplit : MinConsumption);
      const threshold = (isSalaOnline ? ctx.settings?.thresholdSplit : ctx.settings?.threshold) || 100;
      const isFan = val > (isSalaOnline ? MinConsumptionSplit : MinConsumption) && val <= threshold;
