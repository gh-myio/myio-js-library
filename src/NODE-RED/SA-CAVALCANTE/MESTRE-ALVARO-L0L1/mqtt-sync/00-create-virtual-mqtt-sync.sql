-- =============================================================================
-- MESTRE ÁLVARO L0L1 (Sá Cavalcante) — Produto VIRTUAL "MQTT Sync" (plug)
-- Central: Mestre Álvaro L0L1 · CENTRAL_UUID 45250d44-bad0-4071-aaa0-8091cfb12691
-- IPv6 (Yggdrasil): 200:ba5f:dacb:b278:8f85:acf4:f33c:f485
...
--     especializado 'MQTT Sync - 45250d44-bad0-4071-aaa0-8091cfb12691'
...
--   UPDATE slaves   SET name = 'MQTT Sync - 45250d44-bad0-4071-aaa0-8091cfb12691', updated_at = now() WHERE name = 'MQTT Sync';
--   UPDATE channels SET name = 'MQTT Sync - 45250d44-bad0-4071-aaa0-8091cfb12691', updated_at = now() WHERE name = 'MQTT Sync';
--   UPDATE ambients SET name = 'MQTT Sync - 45250d44-bad0-4071-aaa0-8091cfb12691', updated_at = now() WHERE name = 'MQTT Sync';
-- ─────────────────────────────────────────────────────────────────────────────
...
new_slave AS (
  ...
  SELECT
    ...
    'MQTT Sync - 45250d44-bad0-4071-aaa0-8091cfb12691', NULL, '002-002-002-012',
...
new_channel AS (
  ...
  SELECT 'plug', 0, 'MQTT Sync - 45250d44-bad0-4071-aaa0-8091cfb12691', NULL, id, NULL, NULL,
...
new_ambient AS (
  ...
  VALUES ('MQTT Sync - 45250d44-bad0-4071-aaa0-8091cfb12691', NULL, NULL, NULL, now(), now())
  RETURNING id
)
