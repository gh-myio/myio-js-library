-- =============================================================================
-- CAMPINAS G0 NOVO (Soul Malls) — Produto VIRTUAL "MQTT Sync" (plug)
-- Central: Campinas Shopping — G0 Nova · CENTRAL_UUID 401230d1-e7d6-46dd-9bb1-059387683303
-- IPv6 (Yggdrasil): 200:83a1:247a:8c7b:d428:3ed4:21dd:389f
...
--     especializado 'MQTT Sync - 401230d1-e7d6-46dd-9bb1-059387683303'
...
--   UPDATE slaves   SET name = 'MQTT Sync - 401230d1-e7d6-46dd-9bb1-059387683303', updated_at = now() WHERE name = 'MQTT Sync';
--   UPDATE channels SET name = 'MQTT Sync - 401230d1-e7d6-46dd-9bb1-059387683303', updated_at = now() WHERE name = 'MQTT Sync';
--   UPDATE ambients SET name = 'MQTT Sync - 401230d1-e7d6-46dd-9bb1-059387683303', updated_at = now() WHERE name = 'MQTT Sync';
-- ─────────────────────────────────────────────────────────────────────────────
...
new_slave AS (
  ...
  SELECT
    ...
    'MQTT Sync - 401230d1-e7d6-46dd-9bb1-059387683303', NULL, '002-002-002-012',
...
new_channel AS (
  ...
  SELECT 'plug', 0, 'MQTT Sync - 401230d1-e7d6-46dd-9bb1-059387683303', NULL, id, NULL, NULL,
...
new_ambient AS (
  ...
  VALUES ('MQTT Sync - 401230d1-e7d6-46dd-9bb1-059387683303', NULL, NULL, NULL, now(), now())
  RETURNING id
)
