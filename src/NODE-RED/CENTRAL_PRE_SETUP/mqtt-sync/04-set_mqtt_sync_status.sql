...
--   slave:  slave_id explícito; senão o slave virtual "MQTT Sync%" (LIKE —
--           cobre o nome legado 'MQTT Sync' e o especializado por central
--           'MQTT Sync - <CENTRAL_UUID>', ex.: pre-setup a77ac87c-…-0f6f6038e98e)
--
...
BEGIN
  ...
  -- slave alvo: slave_id explícito; senão, o slave virtual "MQTT Sync%" (LIKE)
  v_slave_id := NULLIF(payload->>'slave_id', '')::INTEGER;
  IF v_slave_id IS NULL THEN
    SELECT id INTO v_slave_id FROM slaves WHERE name LIKE 'MQTT Sync%' ORDER BY id LIMIT 1;
  END IF;
  IF v_slave_id IS NULL THEN
