...
function handleDeviceType(name) {
  // Após NFD, todos os diacríticos latinos (acentos, til, cedilha) ficam em U+0300-U+036F.
  // Evita \p{Diacritic} (Unicode property escape) — não suportado em runtimes Node.js mais antigos.
  const upper = (name || '')
    .toUpperCase()
    .normalize('NFD')
    .replace(/[\u0300-\u036f]/g, '');
