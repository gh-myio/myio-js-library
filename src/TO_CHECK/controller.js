...
async function openDashboardPopupEnergi(entityId, entityType, entitySlaveId, entityCentralId, entityLabel, entityComsuption) {
    ...
    const payload = {
        devices: [
            { centralId: gatewayId, slaveId: entitySlaveId },
        ],
        endTs: endDate,
        startTs: startDate,
    ...
    function renderWidget() {
        const displayLabel = attrs.label ? attrs.label.toUpperCase() : labelDefault.toUpperCase();

        // Determinar cor, sinal e seta com base no tipo e valor real
        const sign = percentageType === 'increase' ? '+' : percentageType === 'decrease' ? '-' : '';
        const arrow = percentageType === 'increase' ? '▲' : percentageType === 'decrease' ? '▼' : '';
        const color = percentageType === 'increase' ? '#D32F2F' : percentageType === 'decrease' ? '#388E3C' : '#000';
...
">
    ...
    <div style="text-align:center; font-size:0.85rem; color:#757575; margin-bottom:12px;">
        Último período: <strong>${lastConsumption.toFixed(2)}</strong> ${measurement}
    </div>
...
`);
    ...
    try {
        ...
        percentageType = percentageChange > 0 ? 'increase' : (percentageChange < 0 ? 'decrease' : 'neutral');
    ...
    $("#close-dashboard-popup").on("click", () => $("#dashboard-overlay").remove());
}
    ...
    <div class="popup-content" style="display: flex; justify-content: space-evenly; gap: 10px; padding: 10px; flex: 1; flex-wrap: wrap; box-sizing: border-box; overflow-y: auto;">
        ...
        <div class="card"
            ...
            <div style="flex: 1 1 auto; overflow-y: auto; min-height: 0;">
                <h3 style="color: #4A148C; margin-bottom: 20px;">${valores.etiqueta || ""}</h3>

                <label style="display:block; margin-bottom:4px; font-weight:500; color:#333;">Etiqueta</label>
                <input type="text" class="form-input" value="${valores.etiqueta || ""}" style="width:100%; margin-bottom:16px; padding:8px 10px; font-size:14px; border:1px solid #ccc; border-radius:6px; box-sizing:border-box;" />

                <label style="display:block; margin-bottom:4px; font-weight:500; color:#333;">Andar</label>
                <input type="text" class="form-input" value="${valores.andar || ""}" style="width:100%; margin-bottom:16px; padding:8px 10px; font-size:14px; border:1px solid #ccc; border-radius:6px; box-sizing:border-box;" />

                <label style="display:block; margin-bottom:4px; font-weight:500; color:#333;">Número da Loja</label>
                <input type="text" class="form-input" value="${valores.numeroLoja || ""}" style="width:100%; margin-bottom:16px; padding:8px 10px; font-size:14px; border:1px solid #ccc; border-radius:6px; box-sizing:border-box;" />

                <label style="display:block; margin-bottom:4px; font-weight:500; color:#333;">Identificador do Medidor</label>
                <input type="text" class="form-input" value="${valores.identificadorMedidor || ""}" style="width:100%; margin-bottom:16px; padding:8px 10px; font-size:14px; border:1px solid #ccc; border-radius:6px; box-sizing:border-box;" />

                <label style="display:block; margin-bottom:4px; font-weight:500; color:#333;">Identificador do Dispositivo</label>
                <input type="text" class="form-input" value="${valores.identificadorDispositivo || ""}" style="width:100%; margin-bottom:16px; padding:8px 10px; font-size:14px; border:1px solid #ccc; border-radius:6px; box-sizing:border-box;" />

                <label style="display:block; margin-bottom:4px; font-weight:500; color:#333;">GUID</label>
                <input type="text" class="form-input" value="${valores.guid || ""}" style="width:100%; margin-bottom:16px; padding:8px 10px; font-size:14px; border:1px solid #ccc; border-radius:6px; box-sizing:border-box;" />
            </div>
        ...
        <div class="card"
            ...
            <div style="flex: 1 1 auto; overflow-y: auto; min-height: 0;">
                <h3 style="color: #4A148C; margin-bottom: 20px;">Alarmes Energia - ${valores.etiqueta}</h3>

                <label style="display:block; margin-bottom:4px; font-weight:500; color:#333;">Consumo Máximo Diário (kWh)</label>
                <input type="text" class="form-input" value="${valores.consumoDiario || ""}" style="width:100%; margin-bottom:16px; padding:8px 10px; font-size:14px; border:1px solid #ccc; border-radius:6px; box-sizing:border-box;" />

                <label style="display:block; margin-bottom:4px; font-weight:500; color:#333;">Consumo Máximo na Madrugada (0h - 06h) (kWh)</label>
                <input type="text" class="form-input" value="${valores.consumoMadrugada || ""}" style="width:100%; margin-bottom:16px; padding:8px 10px; font-size:14px; border:1px solid #ccc; border-radius:6px; box-sizing:border-box;" />

                <label style="display:block; margin-bottom:4px; font-weight:500; color:#333;">Consumo Máximo Horário Comercial (09h - 22h) (kWh)</label>
                <input type="text" class="form-input" value="${valores.consumoComercial || ""}" style="width:100%; margin-bottom:16px; padding:8px 10px; font-size:14px; border:1px solid #ccc; border-radius:6px; box-sizing:border-box;" />
            </div>
...
function updateReportTable() {
    ...
    if (!self.ctx.$scope.reportData || self.ctx.$scope.reportData.length === 0) {
        $reportBody.append('<tr><td colspan="3" class="no-data">Nenhum dado disponível</td></tr>');
        return;
    }
        ...
        if (device.isValid) {
            $row = $(`
                <tr>
                    ...
                    <td>${deviceId}</td>
                    <td>${device.consumptionKwh != null ? device.consumptionKwh.toFixed(2) : "-"}</td>
                </tr>
            `);
        } else {
            $row = $(`
...
function exportToCSVAll(reportData ) {
    ...
    link.setAttribute("download", `relatorio_consumo_geral_por_loja_${new Date().toISOString().slice(0, 10)}.csv`);
...
`);
    ...
    const attributeFetchPromises = datasources.map(async (ds) => {
        ...
        try {
            ...
            const deviceIdAttr = deviceAttributes.find(attr => attr.key === 'deviceId');
    ...
    $("#loadDataBtn").on("click", async () => {
        ...
        try {
            const response = await fetch(`https://ingestion.myio-bas.com/api/v1/energy-readings/batch-period-sum`, {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json'
                },
            ...
            self.ctx.$scope.reportData.forEach(device => {
                const match = data.find(d => d.centralId === device.centralId && d.slaveId === device.slaveId);
    ...
    function updateCardOrAdd(group, entityId, label, val, $card) {
        const $existingCard = $(`#dashboard-popup .device-card-centered[data-entity-id="${entityId}"]`);
...
function getDateRangeArray(start, end) {
    ...
    while (currentDate <= endDate) {  // <= para incluir o último dia
...
function exportToCSV(reportData, entityLabel, totalconsumption, entityUpdatedIdentifiers, insueDate) {
    if (!reportData?.length) {
        alert('Erro: Nenhum dado disponível para exportar.');
        return;
    }
    ...
    const link = document.createElement("a");
    link.setAttribute("href", encodeURI(csvContent));
    link.setAttribute("download", `relatorio_consumo_${new Date().toISOString().slice(0, 10)}_${entityLabel}.csv`);
    document.body.appendChild(link);
    link.click();
    document.body.removeChild(link);
}
...
function habilitarBotaoExport() {
    const btn = document.getElementById("btn-export-csv") || document.getElementById("exportCsvBtn");
    btn.disabled = false;
...
function openDashboardPopupReport(entityId, entityType, entitySlaveId, entityCentralId, entityLabel, entityComsuption, entityUpdatedIdentifiers) {
    const insueDate =
        $("#dashboard-popup").remove();
...
`);
    ...
    function updateTable() {
        ...
        data.forEach((item, index) => {
            ...
            tr.innerHTML = `
        <td style="padding: 8px 12px; color: ${corTexto}; background-color: ${corFundo}; text-shadow: 0 2px 4px rgba(0, 0, 0, 0.5)">${item.date}</td>
        <td style="padding: 8px 12px; color: ${corTexto}; background-color: ${corFundo};text-shadow: 0 2px 4px rgba(0, 0, 0, 0.5)">${item.consumptionKwh.toFixed(2)}</td>
    `;
    ...
    self.ctx.$scope.loadData = async function () {
        ...
        try {
            ...
            document.getElementById('total-consumo').textContent = totalconsumption.toFixed(2) + " kWh";
    ...
    $("#btn-export-csv").on("click", () => {
        if (self.ctx.$scope.reportData) {
            ...
            exportToCSV(self.ctx.$scope.reportData, entityLabel, self.ctx.$scope.totalConsumption, entityUpdatedIdentifiers, self.ctx.$scope.insueDate)
        } else {
...
self.onInit = async function () {
    ...
    ctx.$container.on("click", ".card-action", function (e) {
        ...
        if (action === "dashboard") openDashboardPopupEnergi(entityId, entityType, entitySlaveId, entityCentralId, entityLabel, entityComsuption);
        else if (action === "report")
            openDashboardPopupReport(entityId, entityType, entitySlaveId, entityCentralId, entityLabel, entityComsuption, entityUpdatedIdentifiers);
...
function classify(label) {
  ...
  // Infra predial
  if (/administra|adm\.?|bomba|chiller/.test(l)) return "Administração e Bombas";
...
self.onDataUpdated = async function () {
    ...
    items.forEach(
        ({ entityId, entityType, label, val, slaveId, centralId, sourceName }) => {
            ...
            const $card = $(`
      <div class="device-card-centered clickable" data-entity-id="${entityId}" data-entity-label="${labelOrName}" data-entity-type="${entityType}" data-entity-slaveid="${slaveId}" data-entity-centralid="${centralId}" data-entity-updated-identifiers='${JSON.stringify(updatedIdentifiers)}'>
    ...
    `);
            ...
            const $existingCard = ctx.groupDivs[group]
                .find(`.device-card-centered[data-entity-id="${entityId}"]`);
