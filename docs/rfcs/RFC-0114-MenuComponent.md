...
This RFC documents the reverse-engineering of the existing MENU widget (`src/thingsboard/MYIO-SIM/v5.2.0/MENU/`) and proposes extracting its functionality into a reusable **MenuComponent** in the MYIO library. The menu provides tab navigation, context modals, date range selection, and action buttons for the Head Office dashboard.
...
The existing MENU widget (`src/thingsboard/MYIO-SIM/v5.2.0/MENU/`) is a ThingsBoard widget that provides:
...
```
src/thingsboard/MYIO-SIM/v5.2.0/MENU/
...
- `src/thingsboard/MYIO-SIM/v5.2.0/MENU/controller.js` (~900+ lines)
