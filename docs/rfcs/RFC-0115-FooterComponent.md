...
This RFC documents the reverse-engineering of the existing FOOTER widget (`src/thingsboard/MYIO-SIM/v5.2.0/FOOTER/`) and proposes extracting its functionality into a reusable **FooterComponent** in the MYIO library. The footer provides a selection dock for comparing devices, displaying selected items as chips, and opening comparison modals.
...
The existing FOOTER widget (`src/thingsboard/MYIO-SIM/v5.2.0/FOOTER/`) is a ThingsBoard widget that provides:
...
```
src/thingsboard/MYIO-SIM/v5.2.0/FOOTER/
...
- `src/thingsboard/MYIO-SIM/v5.2.0/FOOTER/controller.js` (~1527 lines)
