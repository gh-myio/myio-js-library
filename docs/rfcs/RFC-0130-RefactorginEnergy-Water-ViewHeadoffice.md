...
- `src/thingsboard/MYIO-SIM/v5.2.0/MAIN/controller.js`
- `src/thingsboard/MYIO-SIM/v5.2.0/ENERGY/controller.js`
- `src/thingsboard/MYIO-SIM/v5.2.0/WATER/controller.js`
...
```js
fullscreenModalInstance = (
  await window.MyIOUtils.openConsumptionFullscreen({
    domain: 'energy',
...
## C.1 ENERGY — `src/thingsboard/MYIO-SIM/v5.2.0/ENERGY/controller.js`
...
```js
cleanupFns.push(
  window.MyIOUtils.onOrchestratorReady((orch) => {
    orch.requestSummary?.();
  })
);
```
...
```js
cleanupFns.push(
  window.MyIOUtils.addListenerBoth('myio:energy-summary-ready', registeredHandlers.handleEnergySummary)
);
cleanupFns.push(
  window.MyIOUtils.addListenerBoth('myio:filter-applied', registeredHandlers.handleFilterApplied)
);
cleanupFns.push(
...
## C.2 WATER — `src/thingsboard/MYIO-SIM/v5.2.0/WATER/controller.js`
...
# Plano de Refactoring - MYIO-SIM v5.2.0

## Objetivo

Extrair funções duplicadas de ENERGY e WATER controllers para `window.MyIOUtils` e `window.MyIOOrchestrator` no MAIN, padronizando:

- Compatibilidade ThingsBoard (`$id`)
- Registro/cleanup de eventos
- Readiness do orchestrator (sem polling)
- Fullscreen modal unificado
...
| ------------------------------------------ | ------ | ------------------- |
| `src/thingsboard/MYIO-SIM/v5.2.0/MAIN/controller.js`   | ~7400  | Adicionar utilities |
| `src/thingsboard/MYIO-SIM/v5.2.0/ENERGY/controller.js` | ~1855  | Consumir utilities  |
| `src/thingsboard/MYIO-SIM/v5.2.0/WATER/controller.js`  | ~1636  | Consumir utilities  |

---

## PR 1: Infraestrutura de Utils (Sem Mudar Lógica)

### 1.1 Adicionar `MyIOUtils.$id(ctx, id)` no MAIN

**Local:** Após linha 100 em `MAIN/controller.js`
...
**Substituições em WATER:**

- Remover função `$id` local (linhas 97-103)
- Adicionar no topo: `const $id = (id) => window.MyIOUtils.$id(self.ctx, id);`

**Substituições em ENERGY:**

- Adicionar wrapper: `const $id = (id) => window.MyIOUtils.$id(self.ctx, id);`
- Substituir ~15 usos de `document.getElementById()`:
  - Linhas 119-120, 145-146, 176-178, 192-194, 203-205 (cards)
  - Linha 316, 326 (chart overlay)
  - Linha 806 (maximize button)

---

### 1.2 Adicionar `MyIOUtils.addListenerBoth(eventName, handler, options)` no MAIN

**Local:** Após `$id` no MAIN
...
// No onInit
cleanupAll(); // Limpa antes de registrar
cleanupFns.push(window.MyIOUtils.addListenerBoth('myio:filter-applied', handler));
...
## PR 2: Orchestrator Ready (Substituir Polling)

### 2.1 Adicionar `MyIOUtils.onOrchestratorReady(cb, opts)` no MAIN

**Local:** Após `addListenerBoth`
...
### 2.2 Substituições em ENERGY

**Remover:** Função `waitForOrchestratorAndRequestSummary` (linhas 1758-1784)
...
### 2.3 Substituições em WATER

**Remover:** Os 4 `setTimeout` em cascata no `onInit` (linhas 1512-1622)
...
## PR 3: Water Summary "Sob Demanda"

### 3.1 Adicionar `requestWaterSummary()` no MyIOOrchestrator (MAIN)

**Local:** Após `requestSummary()` (linha ~7382)
...
requestWaterSummary: () => {
  ...
  // Calculate totals
  const commonArea = itemsExcludingEntrada
    .filter(i => i._classification === 'areacomum')
    .reduce((sum, i) => sum + (i.value || 0), 0);

  const stores = itemsExcludingEntrada
    .filter(i => i._classification === 'loja')
    .reduce((sum, i) => sum + (i.value || 0), 0);

  const total = commonArea + stores;

  // Build shopping breakdown (optional)
  ...
  window.dispatchEvent(new CustomEvent('myio:water-summary-ready', { detail: waterSummary }));
  LogHelper.log(`[Orchestrator] requestWaterSummary: emitted (total: ${total.toFixed(2)} m³)`);
},
```

### 3.2 Refatorar WATER para consumir `myio:water-summary-ready`

**Substituir handler `handleWaterDataReady` por `handleWaterSummaryReady`:**

```javascript
function handleWaterSummaryReady(ev) {
  const { stores, commonArea, filteredTotal } = ev.detail || {};
  console.log('[WATER] Received water-summary-ready:', { stores, commonArea, filteredTotal });
...
- Disparos de `myio:request-water-data` (linhas 904-908, 1555-1559, 1583-1587)
- Handler `handleWaterDataReady` (substituído)
- Fallback 60/40 (não mais necessário)

### 3.3 Atualizar `handleDateUpdate` em WATER

```javascript
function handleDateUpdate(event) {
  console.log('[WATER] Date update received:', event.detail);
  initializeCards(); // Loading state
  ...
  if (consumptionChartInstance?.refresh) {
    consumptionChartInstance.refresh(true);
  }
}
```
...
## PR 4: Fullscreen Modal Unificado

### 4.1 Adicionar `MyIOUtils.openConsumptionFullscreen(params)` no MAIN

**Local:** Após `onOrchestratorReady`
...
### 4.2 Substituir `openFullscreenModal()` em ENERGY

```javascript
async function openFullscreenModal() {
  try {
    const result = await window.MyIOUtils.openConsumptionFullscreen({
      domain: 'energy',
      title: 'Consumo de Energia',
      unit: 'kWh',
      unitLarge: 'MWh',
    ...
    fullscreenModalInstance = result.modal;
    isChartFullscreen = true;
  } catch (error) {
    LogHelper.error('[ENERGY] Failed to open fullscreen modal:', error);
  }
}
```

### 4.3 Substituir `openFullscreenModal()` em WATER
...
async function openFullscreenModal() {
  try {
    ...
    console.error('[WATER] Failed to open fullscreen modal:', error);
  }
}
```

### 4.4 Remover código legacy fullscreen

**ENERGY:** Remover `openFullscreenChart()` (linhas 394-563) após estabilização
**WATER:** Remover `openFullscreenChart()` (linhas 975-1186) após estabilização
...
```
PR 1: Infra Utils ($id + addListenerBoth)
  └── Sem mudança de comportamento
  └── Validar que widgets continuam funcionando

PR 2: Orchestrator Ready
  └── Depende de PR 1 (usa addListenerBoth)
  └── Remove polling do ENERGY

PR 3: Water Summary
  └── Depende de PR 1 e PR 2
  └── Adiciona requestWaterSummary no Orchestrator
  └── Remove myio:request-water-data do WATER
  └── Remove fallback 60/40

PR 4: Fullscreen Unificado
  └── Pode ser paralelo aos PRs 2-3
  └── Remove ~600 linhas de código legacy
```
...
| Risco                                                   | Mitigação                                            |
| ------------------------------------------------------- | ---------------------------------------------------- |
| `window.MyIOUtils` não disponível no load do widget     | Manter fallback local mínimo ou checar antes de usar |
| Diferença de regras de total (água exclui entrada)      | Centralizar cálculo no Orchestrator                  |
| Duplicação de listeners se onInit rodar múltiplas vezes | Chamar `cleanupAll()` no início do onInit            |
| Ordem de scripts                                        | MAIN deve carregar antes de ENERGY/WATER             |

---

## Smoke Tests Manuais

1. [ ] Cards saem de loading sem reload da página
2. [ ] Filtro de shopping atualiza charts corretamente
3. [ ] Fullscreen abre/fecha sem memory leaks (reabrir várias vezes)
4. [ ] WATER mostra totais corretos (sem 60/40)
5. [ ] Trocar período (7/14/30) refaz fetch no chart
6. [ ] Orchestrator ready funciona com carregamento tardio
