...
Migrate the ThingsBoard-coupled WATER widget (`src/thingsboard/MYIO-SIM/v5.2.0/WATER/controller.js`) to a standalone, reusable TypeScript component (`src/components/water-panel/`) following the architecture established by `telemetry-grid` and `energy-panel`. This enables:
